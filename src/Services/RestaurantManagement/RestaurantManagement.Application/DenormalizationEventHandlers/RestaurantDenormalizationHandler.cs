...

{
    ...
    private readonly IMenuCategoryQueryRepository _menuCategoryRepository;
    private readonly IFoodTypeQueryRepository _foodTypeRepository;
    private readonly ILogger<RestaurantDenormalizationHandler> _logger;

    public RestaurantDenormalizationHandler(
        IRestaurantQueryRepository repository,
        IMenuCategoryQueryRepository menuCategoryRepository,
        IFoodTypeQueryRepository foodTypeRepository,
        ...
        RestaurantCreatedDenormalizationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        var restaurant = new RestaurantQuery
        {
            Id = @event.Id,
            OwnerId = @event.OwnerId,
            Name = @event.Name,
            Status = @event.Status,
            Address = new AddressQuery(@event.Street, @event.City, @event.State, @event.ZipCode),
            Menus = [],
        };
        ...
        RestaurantUpdatedDenormalizationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        restaurant.Address = new AddressQuery(@event.Street, @event.City, @event.State, @event.ZipCode);
    ...
    public async Task HandleAsync(MenuCreatedDenormalizationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        restaurant.Menus.Add(new RestaurantMenuQuery
        {
            Id = @event.Id,
        ...
        MenuItemAddedDenormalizationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        menu.MenuItems.Add(new RestaurantMenuItemQuery
        {
            Id = @event.Id,
            ...
            Stock = @event.Stock,
            FoodTypes = foodTypes.Select(s => new RestaurantFoodTypesQuery() { Id = s.Id, Name = s.Name }).ToArray(),
        });
        ...
        MenuItemFoodTypesUpdatedDenormalizationEvent @event, CancellationToken cancellationToken = default)
    {
        ...
        menuItem.FoodTypes = foodTypes
            .Select(s => new RestaurantFoodTypesQuery() { Id = s.Id, Name = s.Name })
            .ToArray();
    ...
    private async ValueTask<RestaurantQuery> GetRestaurantQuery(
    ...
    private RestaurantMenuQuery GetMenu(RestaurantQuery restaurantQuery, Guid menuId, string eventName)
    {
        var menu = restaurantQuery.Menus.SingleOrDefault(m => m.Id == menuId);
        if (menu == null)
        {
            _logger.LogError(
    ...
    private RestaurantMenuItemQuery GetMenuItem(RestaurantQuery restaurantQuery, Guid menuItemId, string eventName)
    {
        var menuItem = restaurantQuery.Menus.SelectMany(s => s.MenuItems).SingleOrDefault(m => m.Id == menuItemId);
        if (menuItem == null)
        {
            _logger.LogError(
    ...
    private async ValueTask<MenuCategoryQuery> GetMenuCategory(
        Guid id, string eventName, CancellationToken cancellationToken = default)
    {
    ...
    private async ValueTask<ICollection<FoodTypeQuery>> GetFoodTypes(
